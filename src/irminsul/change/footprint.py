...
class Footprint:
    ...
    """Component doc id -> changed files its recommended tests name.

    That a recommended test file changed. Not that it ran, not that it passed, and not
    that it covers what the change did — nothing in Irminsul executes a test."""
...
def touched_components(
    ...
    for node in graph.nodes.values():
        hits = matching_paths(node.frontmatter.effective_recommended_tests, list(changed_paths))
        if hits:
