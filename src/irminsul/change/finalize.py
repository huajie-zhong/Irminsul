"""`change finalize`: the only transition from accepted to implemented.
...
from irminsul.checks.pipeline import enabled_findings, without_baselined
...
def plan_finalize(
    ...
    *,
    ...
    already_implemented = fm.rfc_state == RfcStateEnum.implemented
    if not already_implemented and fm.rfc_state != RfcStateEnum.accepted:
        blockers.append(
            Blocker(
                code="invalid-state",
                message=(
                    f"only an accepted RFC can be finalized; this one is {fm.rfc_state.value}"
                ),
                path=rfc_path,
    ...
    for finding in _check_errors(graph, enabled_findings(graph), node, None, decision_updates=True):
        blockers.append(
            Blocker(
    ...
    if not blockers:
        ...
        for target in RfcFollowThroughCheck().missing_implements(graph, node):
            if target.id in backlinked:
                continue
            backlinked.add(target.id)
            component_fixes.append(
                Fix(
                    path=target.path,
        ...
        if not already_implemented:
            rfc_fixes.append(
                Fix(
                    ...
                    apply=setter("rfc_state", RfcStateEnum.implemented.value),
                    requires_confirm=True,
                )
            )
...
def _decision_update_blockers(graph: DocGraph, node: DocNode) -> list[Blocker]:
    ...
    An `update-missing-implements` is not a blocker: adding that `implements` entry to
    every required-update doc is exactly what finalization does, and blocking on it would
    make the write that resolves it unreachable.
    """
    out: list[Blocker] = []
    for finding in without_baselined(graph, RfcFollowThroughCheck().run(graph)):
        ...
        if not relates or finding.category not in (
            ...
            continue
        if finding.category == "update-missing-implements":
            continue
        out.append(
            Blocker(
