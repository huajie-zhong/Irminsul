...
Glossary enforcement is opt-in per term: a heading in
...
#: An HTML comment is not rendered prose, and two kinds of it are load-bearing:
#: `<!-- anchor: path#symbol @sha256:... -->` seals a claim, and an `irminsul:ignore`
#: comment records why a finding was judged wrong. Auto-linking a glossary term inside
#: either one corrupts it — an anchor whose path gained a markdown link stops resolving.
...
def _redefinition_line(body: str, term: str) -> int | None:
    """The body line where a doc defines `term` itself: a heading or bold lead-in naming it,
    or a sentence that opens with it and says what it is ("A DocGraph is a ...")."""
    name = re.escape(term)
    pattern = re.compile(
        rf"(?m)^#{{1,6}}\s+{name}\s*$"
        rf"|^\*\*{name}\*\*:?\s*$"
        rf"|(?:\A|\n[ \t]*\n|[.!?][ \t]+|[.!?]\n|\n[ \t]*[-*+][ \t]+)"
        rf"(?:(?:A|An|The)[ \t]+)?{name}[ \t]+(?:is[ \t]+(?:a|an|the)|means|refers[ \t]+to)\b"
    )
    match = pattern.search(body)
    if match is None:
        return None
    start = body.find(term, match.start())
...
class GlossaryDisciplineCheck:
    ...
    explanations: ClassVar[dict[str, str]] = {
        ...
        CODE_REDEFINED_TERM: (
            "A doc other than the glossary seems to define a glossary term: a heading or bold "
            "lead-in naming it, or a sentence such as 'A DocGraph is a ...'. The glossary is "
            "canonical, so link to it; a sentence that only describes the term can keep a "
            "reasoned ignore comment."
        ),
    ...
    classes: ClassVar[dict[str, FindingClass]] = {
        CODE_MALFORMED_METADATA: FindingClass.certain,
        CODE_REDEFINED_TERM: FindingClass.hint,
        CODE_FORBIDDEN_SYNONYM: FindingClass.certain,
        CODE_UNLINKED_TERM: FindingClass.hint,
        CODE_UNUSED_TERM: FindingClass.hint,
    }
        ...
        for entry in entries:
            ...
            for cache in node_caches:
                ...
                if node.path.as_posix() == glossary_path_posix:
                    continue
                line = _redefinition_line(cache.masked_body, entry.term)
                if line is not None:
                    out.append(
                        Finding(
                            check=self.name,
                            ...
                            suggestion=(
                                f"link to {glossary_path_posix}#{slug} instead, or, if the "
                                "sentence is not a definition, say why in an ignore comment"
                            ),
                        )
                    )
    ...
    def fixes(self, findings: list[Finding], graph: DocGraph) -> list[Fix]:
        """Wrap the first use of a glossary term with its anchor link.
        ...
        for cache in node_caches:
            ...
            out.append(
                Finding(
                    ...
                    path=node.path,
                    doc_id=node.id,
                    line=node.file_line(min(matching_lines)),
    ...
    def _entry_is_unused(
        ...
        for cache in node_caches:
            if anchor in cache.glossary_anchors:
                return False
            for match_term in entry.match_terms:
                # Unmasked: an identifier-spelled term is only ever written in a code
                # span, which the masked body blanks.
                if _has_match(cache.node.body, match_term, case_sensitive=entry.case_sensitive):
                    return False
        return True
