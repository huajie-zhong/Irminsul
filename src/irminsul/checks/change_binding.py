"""ChangeBindingCheck — declared change intent vs. derived footprint.
...
class ChangeBindingCheck:
    ...
    classes: ClassVar[dict[str, FindingClass]] = {
        CODE_MISSING_AFFECTS: FindingClass.certain,
        CODE_UNKNOWN_COMPONENT: FindingClass.certain,
        CODE_TOUCHED_BUT_UNDECLARED: FindingClass.hint,
        CODE_DECLARED_BUT_UNTOUCHED: FindingClass.hint,
    }
    ...
    def run(self, graph: DocGraph) -> list[Finding]:
        ...
        rfcs = [
            ...
            if is_rfc(node, graph.config) and node.frontmatter.rfc_state is not None
        ]
    ...
    def _check_shape(self, graph: DocGraph, node: DocNode) -> list[Finding]:
        ...
        if fm.rfc_state in (RfcStateEnum.accepted, RfcStateEnum.implemented) and fm.affects is None:
            out.append(
                Finding(
                    check=self.name,
                    ...
                    message=(
                        f"{fm.rfc_state.value} RFC does not declare `affects`; "
    ...
    def _check_binding(self, graph: DocGraph, rfcs: list[DocNode]) -> list[Finding]:
        ...
        accepted = [
            ...
            and node.frontmatter.rfc_state == RfcStateEnum.accepted
