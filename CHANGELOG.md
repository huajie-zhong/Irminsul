...
### Enhancements
- `irminsul init` writes the agent-harness wiring alongside the scaffold: a project-scoped `.mcp.json` registering the read-only MCP server, a trigger-only `.claude/skills/irminsul/SKILL.md` that routes an agent to `irminsul orient` and the agent protocol, and a `CLAUDE.md` pointer that imports the root `AGENTS.md`. Existing files are skipped with a note; `--force` merges the `irminsul` entry into an existing registration and keeps every other server (ADR-0023).
...
- The composite Action requests `--format github` by default so findings land as annotations on the failing line, and installs the CLI from the same ref of this repository the workflow references unless `version` pins a release, so its inputs always match the installed CLI. An empty `format` or `profile` input falls back to the CLI default instead of exiting 2.
