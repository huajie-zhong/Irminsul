...
### Enhancements
- `irminsul init` writes the agent-harness wiring alongside the scaffold: a project-scoped `.mcp.json` registering the read-only MCP server, a trigger-only `.claude/skills/irminsul/SKILL.md` that routes an agent to `irminsul orient` and the agent protocol, and a `CLAUDE.md` pointer that imports the root `AGENTS.md` for Claude Code. Existing files are skipped with a note and a hint for whatever is not wired yet; `--force` merges rather than replaces the adopter-owned pair — the `irminsul` entry is set in an existing registration and every other server kept, and a `CLAUDE.md` without the import gains it at the top. A registration that is not a JSON object is never rewritten, and a byte-order mark no longer makes it count as one. In the `siblings` layout the wiring lands in the docs repo and the next steps say how to register the server from the code repo (ADR-0023).
- `irminsul fix --check <name>` rejects a name no registry knows, with a did-you-mean, instead of reporting it as inactive under the profile.
- `irminsul fix --format json` — the versioned envelope of a fix run; its `notes` list carries what a run that harvested nothing would have printed, such as an inactive `--check` name.
- `irminsul orient` reports the root `AGENTS.md` router ahead of the docs-root entries, and `irminsul context` hints reuse the finding-to-fix mapping instead of returning a constant.
- The composite Action requests `--format github` by default so findings land as annotations on the failing line, and installs the CLI from the same ref of this repository the workflow references unless `version` pins a release, so an unpinned run always matches the CLI it installs; a pinned release has to accept `--profile` and `--format`, which v0.2.0 and earlier do not. Inputs and contexts reach the scripts through `env` rather than being expanded into bash source. An empty `format` or `profile` input falls back to the CLI default instead of exiting 2, and CI runs the Action against this repository.
...
### Changed
- The doc walk skips dot-directories under `docs_root` and exempts a top-level `CLAUDE.md`, so a repo whose `docs_root` is the repo root no longer fails the hard gate on the harness files `irminsul init` writes.
- Every file `irminsul init` writes uses LF newlines on every platform; the scaffold templates used to follow the platform convention while the harness files did not.
- `irminsul fix`, `change transition` and `change finalize` list the files a live run wrote rather than the fixes it planned; a dry run still lists the plan. `check --format github` prints the baseline and delta notes the other formats already carried.
- This repository's `CLAUDE.md` imports the root `AGENTS.md` instead of duplicating it, and the root router no longer restates the check registries' counts and names.
