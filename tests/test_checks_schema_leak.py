...
def test_bad_schema_leak_flags_class_in_components(
    ...
    repo = fixture_repo("bad-schema-leak")
    findings = _run(repo)
    ...
    doc_lines = (repo / "docs/components/leaky.md").read_text(encoding="utf-8").splitlines()
    flagged = {doc_lines[f.line - 1] for f in leaky_findings if f.line is not None}
    assert "class Thing(BaseModel):" in flagged
    assert "interface Thing {" in flagged


def test_bad_schema_leak_does_not_flag_outside_protected_glob(
    fixture_repo: Callable[[str], Path],
) -> None:
    """guides/thing.md contains the same class def, but lives outside the
    protected glob (docs/components/**) — must NOT be flagged."""
    findings = _run(fixture_repo("bad-schema-leak"))
    outside_findings = [f for f in findings if f.path and "guides" in f.path.as_posix()]
