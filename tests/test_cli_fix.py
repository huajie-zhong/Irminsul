...
def test_fix_skips_checks_that_are_not_enabled(fixture_repo: Callable[[str], Path]) -> None:
    repo = fixture_repo("soft-supersession")
...
def test_fix_confirm_applies_irreversible_fixes(
    ...
    result = runner.invoke(app, ["fix", "--confirm", "--path", str(repo)])
...
def test_fix_live_run_lists_written_files_only(fixture_repo: Callable[[str], Path]) -> None:
    ...
    result = runner.invoke(app, ["fix", "--confirm", "--format", "json", "--path", str(repo)])

    assert result.exit_code == 0, result.output
    payload = json.loads(result.output)
    assert payload["version"] == 1
    assert payload["dry_run"] is False
    assert payload["written"] == ["docs/components/old-system.md"]
    ...
    assert {entry["path"] for entry in payload["planned"]} == {"docs/components/old-system.md"}
...
def test_fix_check_selector_inactive_name_is_noop(
    ...
    result = runner.invoke(
        app,
        ["fix", "--check", "rfc-follow-through", "--path", str(repo)],
    )
    ...
    # The JSON envelope must differ from a genuine nothing-to-fix run, or an
    # agent following a stale hint reads the finding as resolved.
    result = runner.invoke(
        app,
        [
            "fix",
    ...
    assert payload["notes"] == ["check 'rfc-follow-through' is not active under profile 'enabled'"]
...
def test_fix_check_selector_rejects_an_unknown_name(
    ...
    """A typo must not get the same "not active under profile" note as a real
    check outside the profile, or retrying with a wider profile never reveals
