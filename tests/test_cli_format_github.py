...
def test_check_format_github_end_to_end(fixture_repo: Callable[[str], Path]) -> None:
    ...
    result = runner.invoke(app, ["check", "--format", "github", "--path", str(repo)])
...
def test_check_format_github_green_repo_prints_summary_only(
    ...
    # A green repo has history: without it `history-depth` rightly says the checks
    # that read git can see nothing here.
    for args in (
        ("init", "-q"),
        ("add", "-A"),
        ("-c", "user.name=T", "-c", "user.email=t@e.com", "commit", "-qm", "green"),
    ):
...
def test_check_rejects_unknown_format(fixture_repo: Callable[[str], Path]) -> None:
    ...
    assert "plain, json, or github" in cli_output(result)
