"""Tests for binding readiness and its report extensions."""
...
def test_new_rfc_blocked_by_errors(fixture_repo: Callable[[str], Path]) -> None:
    repo = fixture_repo("bad-frontmatter")
    result = runner.invoke(app, ["new", "rfc", "Blocked Idea", "--path", str(repo)])
    assert result.exit_code == 1
    assert "checks report errors" in result.output
    assert not list((repo / "docs").glob("rfcs/*blocked-idea*"))
...
def test_report_blocks_on_unresolved_required_update(repo: Path) -> None:
    _git_init(repo)
    rfc = repo / "docs" / "rfcs" / "0001-accepted-good.md"
    text = rfc.read_text(encoding="utf-8")
    rfc.write_text(
        text.replace(
            "required_updates: []",
            "required_updates:\n  - path: docs/guides/missing.md\n    kind: update",
        ),
        encoding="utf-8",
    )
    report = build_change_report(repo, load(find_config(repo)), "0001-accepted-good", env={})
    assert any(b.code.startswith("rfc-follow-through:") for b in report.blockers)
...
def test_broken_anchor_in_affected_scope_blocks(binding_repo: Path) -> None:
    ...
    _append(binding_repo, "docs/components/auth.md", "<!-- anchor: app/auth/gone.py#login -->")
    _git_init(binding_repo)
    ...
    assert broken[0].path == "docs/components/auth.md"
    assert report.mechanically_ready_for == "none"
...
def test_missing_symbol_anchor_in_affected_scope_blocks(binding_repo: Path) -> None:
    _append(binding_repo, "docs/components/auth.md", "<!-- anchor: app/auth/login.py#logout -->")
    _git_init(binding_repo)
...
def test_stale_anchor_outside_affects_stays_visible(binding_repo: Path) -> None:
    ...
    assert scoped[0]["path"] == "docs/components/billing.md"


def test_a_planned_claim_citing_an_accepted_rfc_does_not_block(binding_repo: Path) -> None:
    """An accepted RFC is not built yet, so a `planned` claim citing it is still true."""
    _git_init(binding_repo)
    billing = binding_repo / "docs" / "components" / "billing.md"
    billing.write_text(
        billing.read_text(encoding="utf-8").replace(
            ...
            "    evidence:\n"
            "      - docs/rfcs/0001-accepted-good.md\n",
        ),
        encoding="utf-8",
    )

    report = _report(binding_repo, "0001-accepted-good")
    assert not [b for b in report.blockers if "sso-live" in b.message]
...
def test_verify_plain_renders_scoped_findings(binding_repo: Path) -> None:
    ...
    assert "[claim-anchor] docs/components/auth.md:" in out
