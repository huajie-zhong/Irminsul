...
def test_invalid_metadata_reports_warning(tmp_path: Path) -> None:
    repo = _write_repo(
        ...
        docs={"docs/components/docgraph.md": "Mentions DocGraph."},
    )
...
def test_forbidden_synonym_is_an_error(tmp_path: Path) -> None:
    repo = _write_repo(
        tmp_path,
        glossary=(
            "# Glossary\n\n"
        ...
        docs={"docs/components/widget.md": "The doc system builds a graph."},
    )
    findings = _run_glossary(repo)
    assert any(
        finding.severity == Severity.error and "forbidden synonym 'doc system'" in finding.message
...
def test_unlinked_term_use_is_info(tmp_path: Path) -> None:
    repo = _write_repo(
        ...
        docs={"docs/components/widget.md": "The DocGraph is built once."},
    )
    findings = _run_glossary(repo)
    assert any(
...
def test_existing_glossary_link_suppresses_info(tmp_path: Path) -> None:
    repo = _write_repo(
        ...
        docs={
            "docs/components/widget.md": ("The [DocGraph](../GLOSSARY.md#docgraph) is built once.")
        },
    )
    findings = _run_glossary(repo)
...
def test_local_glossary_anchor_suppresses_info_for_glossary_doc(tmp_path: Path) -> None:
    repo = _write_repo(
        ...
        docs={},
        glossary_path="docs/components/GLOSSARY.md",
...
def test_unused_declared_term_warns(tmp_path: Path) -> None:
    repo = _write_repo(
        ...
        docs={"docs/components/widget.md": "No matching term here."},
    )
    findings = _run_glossary(repo)
    assert any(
...
def test_plural_forms_must_be_explicit(tmp_path: Path) -> None:
    repo = _write_repo(
        ...
        docs={"docs/components/widget.md": "Doc atoms are separate files."},
    )
    findings = _run_glossary(repo)
...
def _soft_glossary_findings(fixture_repo: Callable[[str], Path]) -> list:
    repo = fixture_repo("soft-glossary")
    return GlossaryDisciplineCheck().run(build_graph(repo, load(find_config(repo))))


def test_term_used_only_in_code_spans_is_not_unused(
    fixture_repo: Callable[[str], Path],
) -> None:
    findings = _soft_glossary_findings(fixture_repo)
    unused = [f for f in findings if "declared but unused" in f.message]
    assert not [f for f in unused if "PlanNode" in f.message]


def test_glossary_entry_quoted_in_a_fence_is_not_a_redefinition(
    fixture_repo: Callable[[str], Path],
) -> None:
    findings = _soft_glossary_findings(fixture_repo)
    redefined = {f.doc_id for f in findings if "redefined" in f.message}
    assert "backticked" not in redefined
    assert "composer" in redefined


def test_a_prose_definition_of_a_term_is_a_redefinition(tmp_path: Path) -> None:
    repo = _write_repo(
        tmp_path,
        glossary="# Glossary\n\n## DocGraph\n\nThe in-memory graph of doc atoms.\n",
        docs={
            "docs/components/widget.md": (
                "Widgets render.\n\nA DocGraph is a list of every Markdown file.\n"
            ),
            "docs/components/other.md": "The DocGraph is built once per run.\n",
        },
    )
    findings = [f for f in _run_glossary(repo) if f.code == "glossary-discipline/redefined-term"]
    assert [(f.doc_id, f.line) for f in findings] == [("widget", 11)]


def test_a_redefinition_hint_can_be_cleared_with_a_reason(tmp_path: Path) -> None:
    ...
    repo = _write_repo(
        ...
        docs={
            "docs/components/widget.md": (
                "A DocGraph is a graph. "
                '<!-- irminsul:ignore glossary-discipline/redefined-term reason="quoted" -->\n'
            ),
        },
    )
    graph = build_graph(repo, load(find_config(repo)))
    assert [
        f for f in run_check(GlossaryDisciplineCheck, graph) if f.code.endswith("redefined-term")
    ] == []


def test_redefinition_search_stays_linear_on_long_blank_runs() -> None:
    import time

    from irminsul.checks.glossary import _redefinition_line

    body = "# Doc\n\n" + " \n" * 20000 + "Nothing here.\n"
    started = time.perf_counter()
    assert _redefinition_line(body, "DocGraph") is None
