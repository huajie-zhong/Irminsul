"""Unit tests for the derived layered impact report."""
...
def test_plan_impact_without_git(repo: Path) -> None:
    ...
    architecture = report.layers["architecture"]
    assert any("login-flow" in o.observation for o in architecture)
    assert all(o.review for o in architecture)

    decisions = [o.observation for o in report.layers["decisions"]]
    assert any("docs/decisions/0001-adr.md" in text for text in decisions)
...
def test_observed_impact_lists_changed_docs_by_layer(repo: Path) -> None:
    _git_init(repo)
    flow = repo / "docs" / "architecture" / "login-flow.md"
    flow.write_text(flow.read_text(encoding="utf-8") + "\nUpdated.\n", encoding="utf-8")
    report = build_impact_report(repo, load(find_config(repo)), _RFC, env={})
    assert any(
        "docs/architecture/login-flow.md" in o.observation for o in report.layers["architecture"]
    )


def test_observed_impact_lists_changed_component_docs(repo: Path) -> None:
    _git_init(repo)
    component = repo / "docs" / "components" / "auth.md"
    component.write_text(component.read_text(encoding="utf-8") + "\nUpdated.\n", encoding="utf-8")
    report = build_impact_report(repo, load(find_config(repo)), _RFC, env={})
    [observation] = [
        o for o in report.layers["components"] if o.source == "diff:docs/components/auth.md"
    ]
...
def test_undeclared_changed_component_doc_is_a_divergence(repo: Path) -> None:
    _git_init(repo)
    component = repo / "docs" / "components" / "billing.md"
    component.write_text(component.read_text(encoding="utf-8") + "\nUpdated.\n", encoding="utf-8")
    report = build_impact_report(repo, load(find_config(repo)), _RFC, env={})
    [observation] = [
        o for o in report.layers["components"] if o.source == "diff:docs/components/billing.md"
    ]
...
def test_new_component_doc_surfaces_in_components(repo: Path) -> None:
    _git_init(repo)
    (repo / "docs" / "components" / "sso.md").write_text(
        "---\n"
        "id: sso\n"
        "title: SSO\n"
        ""
    ...
    report = build_impact_report(repo, load(find_config(repo)), _RFC, env={})
    assert any(
        "docs/components/sso.md" in o.observation and "component 'sso'" in o.observation
...
def test_removed_component_doc_surfaces_in_architecture(repo: Path) -> None:
    _git_init(repo)
    (repo / "docs" / "components" / "billing.md").unlink()
    report = build_impact_report(repo, load(find_config(repo)), _RFC, env={})
    [observation] = [o for o in report.layers["architecture"] if o.source.startswith("diff:")]
    assert "removed or moved" in observation.observation
    assert "docs/components/billing.md" in observation.observation
    assert observation.review is not None
