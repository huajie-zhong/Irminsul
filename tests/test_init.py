...
def test_init_no_interactive_creates_expected_tree(tmp_path: Path) -> None:
    ...
    expected = [
        ...
        "docs/decisions/adopt-irminsul.md",
        "docs/rfcs/INDEX.md",
...
def test_init_fresh_generated_repo_passes_hard_check(tmp_path: Path) -> None:
    ...
    check_result = runner.invoke(app, ["check", "--path", str(target)])
...
def test_init_fresh_errors_when_code_signals_exist_without_override(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 2
    assert "--allow-existing-code" in cli_output(result)
...
def test_init_creates_agent_manifests(tmp_path: Path) -> None:
    ...
    assert "irminsul context --after-edit" in root_manifest
    assert "irminsul check" in root_manifest
...
def test_init_writes_trigger_only_skill(tmp_path: Path) -> None:
    ...
    assert "docs/guides/agent-protocol.md" in skill
...
def test_init_skips_the_manual_command_when_already_registered(tmp_path: Path) -> None:
    """A re-run over an adopted repo does not print the manual registration
    command when `.mcp.json` already names the server."""
    target = tmp_path / "demo"
    target.mkdir()
    (target / "src").mkdir()  # code signal
    existing = target / ".mcp.json"
...
def test_init_force_merges_into_an_existing_registration(tmp_path: Path) -> None:
    """`--force` must not replace `.mcp.json` wholesale and delete every other
    server the adopter registered. The file is the adopter's client config, not
    an irminsul-owned template, so the entry is merged in."""
    target = tmp_path / "demo"
    target.mkdir()
    (target / "src").mkdir()  # code signal
    existing = target / ".mcp.json"
...
def test_init_never_rewrites_a_registration_it_cannot_parse(tmp_path: Path) -> None:
    """A file that is not a JSON object may still be the adopter's config —
    JSON with comments, say — so even `--force` leaves it alone, names it, and
...
def test_init_force_merges_into_a_registration_with_a_bom(tmp_path: Path) -> None:
    ...
    rejects. The file must still merge rather than count as unparseable and be
    rewritten with only the `irminsul` entry."""
    target = tmp_path / "demo"
    target.mkdir()
    (target / "src").mkdir()  # code signal
    existing = target / ".mcp.json"
...
def test_init_does_not_clobber_an_existing_claude_md(tmp_path: Path) -> None:
    """A pre-existing `CLAUDE.md` is skipped with a note, so the adopter learns
    Claude Code sessions are not reaching the router."""
    target = tmp_path / "demo"
    target.mkdir()
    (target / "src").mkdir()  # code signal
...
def test_init_force_links_an_existing_claude_md_instead_of_replacing_it(tmp_path: Path) -> None:
    """`--force` must not overwrite an adopter's hand-written `CLAUDE.md`. The
    file is the adopter's, so only the router import is added, at the top, and
    the content and line endings survive."""
    target = tmp_path / "demo"
    target.mkdir()
    (target / "src").mkdir()  # code signal
    custom = target / "CLAUDE.md"
...
def test_tracked_harness_files_match_the_scaffold() -> None:
    ...
    page = (repo_root / "docs" / "components" / "mcp-server.md").read_text(encoding="utf-8")
    registrations = []
...
def test_init_force_overwrites_existing_files(tmp_path: Path) -> None:
    ...
    # Not in git, so replacing it would lose it for good: the old text is kept beside it.
    backup = docs_dir / "principles.md.irminsul-backup"
    assert backup.read_text(encoding="utf-8") == "# my custom principles\n"
    assert "principles.md.irminsul-backup" in unstyle(result.output)


def _force_init(target: Path):
    return runner.invoke(
        app,
        ["init", "--language", "python", "--no-interactive", "--force", "--path", str(target)],
    )


def _git_repo_with_principles(tmp_path: Path, text: str) -> Path:
    import subprocess

    target = tmp_path / "demo"
    (target / "src").mkdir(parents=True)
    (target / "docs" / "foundation").mkdir(parents=True)
    (target / "docs" / "foundation" / "principles.md").write_text(text, encoding="utf-8")
    for args in (
        ["init", "-q"],
        ["add", "-A"],
        ["-c", "user.name=T", "-c", "user.email=t@example.com", "commit", "-qm", "authored"],
    ):
        subprocess.run(["git", "-C", str(target), *args], check=True, capture_output=True)
    return target


def test_init_force_replaces_a_committed_doc_and_says_git_holds_it(tmp_path: Path) -> None:
    target = _git_repo_with_principles(tmp_path, "# committed principles\n")

    result = _force_init(target)

    assert result.exit_code == 0, result.output
    principles = target / "docs" / "foundation" / "principles.md"
    assert "committed principles" not in principles.read_text(encoding="utf-8")
    assert not (principles.parent / "principles.md.irminsul-backup").exists()
    output = unstyle(result.output)
    assert "docs/foundation/principles.md" in output
    assert "git history" in output


def test_init_force_backs_up_a_doc_with_uncommitted_edits(tmp_path: Path) -> None:
    """Committed once, then edited: git holds only the old version, not the edits."""
    target = _git_repo_with_principles(tmp_path, "# committed principles\n")
    principles = target / "docs" / "foundation" / "principles.md"
    principles.write_text("# committed principles\n\nAn edit nobody committed.\n")

    result = _force_init(target)

    assert result.exit_code == 0, result.output
    backup = principles.parent / "principles.md.irminsul-backup"
    assert "An edit nobody committed." in backup.read_text(encoding="utf-8")


def test_init_force_leaves_scaffold_output_that_did_not_change(tmp_path: Path) -> None:
    target = tmp_path / "demo"
    (target / "src").mkdir(parents=True)
    assert _force_init(target).exit_code == 0

    result = _force_init(target)

    assert result.exit_code == 0, result.output
    assert not list(target.rglob("*.irminsul-backup"))


def test_workflow_pins_follow_the_installed_version(monkeypatch: pytest.MonkeyPatch) -> None:
    """A hardcoded pin is right for exactly one release and wrong after it."""
    import irminsul
    from irminsul.init.command import workflow_pins

    monkeypatch.setattr(irminsul, "__version__", "0.4.2")
    assert workflow_pins() == ("v0.4.2", '"irminsul==0.4.2"')


def test_a_development_build_pins_nothing(monkeypatch: pytest.MonkeyPatch) -> None:
    """A dev version names no release PyPI has, so writing it would be a broken pin."""
    import irminsul
    from irminsul.init.command import workflow_pins

    monkeypatch.setattr(irminsul, "__version__", "0.4.2.dev17+gabc1234")
    assert workflow_pins() == ("main", "irminsul")


def test_scaffolded_workflow_carries_no_hardcoded_version(tmp_path: Path) -> None:
    target = tmp_path / "demo"
    (target / "src").mkdir(parents=True)
    result = runner.invoke(
        app, ["init", "--no-interactive", "--language", "python", "--path", str(target)]
    ...
    for name in ("docs-pr.yml", "docs-nightly.yml"):
        text = (target / ".github" / "workflows" / name).read_text(encoding="utf-8")
        assert "0.3.0" not in text, f"{name} still names a version the release will outgrow"


def test_scaffolded_pr_workflows_watch_every_file_the_gate_judges(tmp_path: Path) -> None:
    """A `paths:` filter that misses a file lets a pull request touching only that file
    skip the gate: a second source root, a guidance file, the baseline, the adoption
    record, or another workflow that also runs the check.

    The adoption record is the one that bites hardest, because the check that judges it
    is the only thing standing between a hand-written entry and a permanent exception.
    """
    from irminsul.init.command import InitAnswers, Topology, write_scaffold

    same_repo = tmp_path / "same"
    write_scaffold(
        same_repo,
        InitAnswers(
            project_name="demo",
    ...
    for root, source_filters in ((same_repo, ["src/**", "app/**", "lib/**"]), (siblings, [])):
        workflow = (root / ".github" / "workflows" / "docs-pr.yml").read_text(encoding="utf-8")
        for pattern in (
            *source_filters,
            "docs/**",
            "irminsul.toml",
            ...
            "AGENTS.md",
            "CLAUDE.md",
            ".irminsul-baseline.json",
            ".irminsul-adoption.json",
            ".github/workflows/**",
        ):
            assert f'- "{pattern}"' in workflow, f"{root.name}: {pattern} not watched"


def test_a_root_level_source_root_watches_the_whole_repository(tmp_path: Path) -> None:
    """Go's profile can pick `.`, and `./**` is not a path GitHub matches."""
    ...
    workflow = (tmp_path / ".github" / "workflows" / "docs-pr.yml").read_text(encoding="utf-8")
    assert '- "**"' in workflow
    assert "./**" not in workflow


def test_a_working_launcher_raises_no_mcp_warning(monkeypatch: pytest.MonkeyPatch) -> None:
    """`.mcp.json` names a bare command so the file stays committable, which means the
    harness resolves it on PATH. Init tries it once, because a harness cannot."""
    from irminsul.init import command as init_command

    monkeypatch.setattr(init_command.shutil, "which", lambda _: r"C:\bin\irminsul.exe")
    monkeypatch.setattr(
        init_command.subprocess,
        "run",
        lambda *a, **k: subprocess.CompletedProcess(a[0], 0, "irminsul 0.3.0\n", ""),
    )

    assert init_command.check_mcp_launcher() is None


def test_an_absent_launcher_is_reported(monkeypatch: pytest.MonkeyPatch) -> None:
    from irminsul.init import command as init_command

    monkeypatch.setattr(init_command.shutil, "which", lambda _: None)

    assert init_command.check_mcp_launcher() == "no `irminsul` on PATH"


def test_an_orphaned_console_script_is_reported_with_its_reason(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The real failure mode: the script outlives the package that installed it, so it
    dies inside its own generated `__main__` before any Irminsul code runs. No handler
    inside the MCP server can reach that, which is why it is caught here instead.

    The probe asks `mcp --probe` rather than `--version`, because a base CLI without the
    optional extra answers `--version` quite happily and then fails to serve."""
    from irminsul.init import command as init_command

    monkeypatch.setattr(init_command.shutil, "which", lambda _: r"C:\py\Scripts\irminsul.exe")
    monkeypatch.setattr(
        init_command.subprocess,
        "run",
        lambda *a, **k: subprocess.CompletedProcess(
            a[0], 1, "", "Traceback...\nModuleNotFoundError: No module named 'irminsul'\n"
        ),
    )
    # The probe is the serving probe, not `--version`.
    recorded: list[list[str]] = []
    monkeypatch.setattr(
        init_command.subprocess,
        "run",
        lambda *a, **k: (
            recorded.append(list(a[0])),
    ...
    reason = init_command.check_mcp_launcher()

    assert reason is not None
    assert "cannot serve MCP" in reason and "ModuleNotFoundError" in reason


def test_the_mcp_warning_does_not_send_you_back_to_the_broken_command(
    capsys: pytest.CaptureFixture[str],
) -> None:
    """The old advice was `irminsul mcp --path .`, which in this failure mode prints a
    traceback rather than a hint — and asks the broken thing to explain itself."""
    from irminsul.init.command import print_mcp_warning

    print_mcp_warning(r"`C:\py\Scripts\irminsul.exe` exits 1: ModuleNotFoundError")
    printed = capsys.readouterr().out

    assert "irminsul mcp --path ." not in printed
    assert 'python -c "import irminsul, irminsul.mcp_server"' in printed
    assert "pipx install --force irminsul" in printed
