...
def test_update_missing_implements_fix_adds_implements_without_confirm(
    fixture_repo: Callable[[str], Path],
) -> None:
    repo = fixture_repo("soft-lifecycle")
    fixes = _fixes(repo)
    adr = repo / "docs" / "decisions" / "0004-no-backlink-adr.md"

    backlink_fixes = [f for f in fixes if f.path == Path("docs/decisions/0004-no-backlink-adr.md")]
