...
def _build_siblings(
    ...
    *,
    ...
    components = docs_repo_root / "docs" / "components"
...
def _write_claim_doc(repo_root: Path, evidence: str) -> None:
    ...
    `claim-provenance` only inspects `foundation/` and `architecture/`,
...
def test_a_dotfile_in_the_sibling_repo_is_not_source_evidence(tmp_path: Path) -> None:
    ...
    evidence — the in-repo branch refuses the identical spelling, and
...
def test_checks_pass_across_the_repo_boundary(tmp_path: Path) -> None:
    repo_root = _build_siblings(tmp_path)
    result = runner.invoke(app, ["check", "--path", str(repo_root)])
...
def test_claims_resolve_into_the_sibling_code_repo(tmp_path: Path) -> None:
    ...
    repo_root = _build_siblings(tmp_path)
    result = runner.invoke(app, ["context", "docs/components/core.md", "--path", str(repo_root)])
    assert result.exit_code == 0, result.output
...
def test_a_claim_on_a_missing_sibling_file_is_an_error(tmp_path: Path) -> None:
    ...
    repo_root = _build_siblings(tmp_path)
    doc = repo_root / "docs" / "components" / "core.md"
