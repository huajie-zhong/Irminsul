...
def test_regen_docs_surfaces_creates_generated_refs(tmp_path: Path) -> None:
    repo = _make_repo(tmp_path)
    result = runner.invoke(app, ["regen", "--language", "docs-surfaces", "--path", str(repo)])
    assert result.exit_code == 0, result.output
    ...
    assert "generated by irminsul regen --language=docs-surfaces" in (
        out_dir / "cli-commands.md"
