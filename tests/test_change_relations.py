"""Tests for lifecycle-aware RFC relationship graphs."""
...
def test_repository_graph_includes_stateless_and_isolated_rfcs(repo: Path) -> None:
    report = _report(repo)
    nodes = {node.id: node for node in report.nodes}

    assert "component" not in nodes
    assert nodes["0004-stateless"].state is None
...
def test_dependency_statuses_preserve_unknown_edges(repo: Path) -> None:
    ...
    assert statuses["0004-stateless", "0003-leaf"] == "pending"
...
def test_supersession_statuses_and_effective_successor_conflict(repo: Path) -> None:
    ...
    assert statuses["0004-stateless", "0002-mid"] == "unknown"
    assert statuses["0005-rejected", "0001-base"] == "void"
...
def test_focused_query_returns_selected_weak_component(repo: Path) -> None:
    report = _report(repo, focus="0003-leaf", relation="dependency")
...
def test_json_contract_is_stable_and_preserves_null_state(repo: Path) -> None:
    report = _report(repo, focus="0004-stateless")
    first = relation_graph_to_json(report)
    second = relation_graph_to_json(_report(repo, focus="0004-stateless"))
    ...
    stateless = next(node for node in data["nodes"] if node["id"] == "0004-stateless")
    assert stateless["state"] is None
...
def test_cli_graph_json_accepts_an_id_focus_and_exits_zero_with_issues(repo: Path) -> None:
    result = runner.invoke(
        app,
        ["change", "graph", "0006-implemented-bad", "--format", "json", "--path", str(repo)],
    )
...
def test_cli_graph_is_read_only(repo: Path) -> None:
    rfc = repo / "docs" / "rfcs" / "0010-self.md"
    before = rfc.read_bytes()

    result = runner.invoke(app, ["change", "graph", "0010-self", "--path", str(repo)])
