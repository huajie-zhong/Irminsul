...
claims:
  ...
  - id: schema-leak-enabled
    ...
    kind: ci_gate
    relation: follows-evidence
    claim: Schema-leak detection is enabled in this repository and its findings are certain, so CI fails on them.
    evidence:
...
The system never forces the split; the only length warning is for an index doc whose body runs over `length_warning_lines` (300 by default), because an index is navigation. Promotion to a folder is a human judgment call, made when one file genuinely doesn't fit anymore:
