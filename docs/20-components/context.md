...
Topic search is deterministic per-term substring matching (every term must hit, in any field), not fuzzy or semantic search; matches rank by exact-id match, then whole-phrase hit, then how many distinct fields the terms covered. Whitespace is normalized for ranking, and spaces, hyphens, slashes, and underscores are equivalent phrase separators. `--changed` and
