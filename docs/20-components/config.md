...
Generated configs include the stable/useful deterministic sections: paths, hard and soft checks, implemented nested check settings, overrides, and languages. LLM config is supported by the schema but intentionally omitted from the scaffold until LLM checks are part of the normal rollout.
