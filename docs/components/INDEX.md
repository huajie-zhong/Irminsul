...
- [`anchors`](anchors.md) — anchored prose claims pinned to code
- [`doc-atom`](doc-atom.md) — the frontmatter contract every doc carries
- [`git`](git.md) — git helpers for changed paths, commit times, and blame
- [`refs`](refs.md) — backlinks for a doc and the docs that name a symbol
- [`seed`](seed.md) — capture starting intent into the foundation docs
- [`surface`](surface.md) — code surfaces derived on demand
- [Adoption record](adoption-record.md)

## Scope & Limitations

This is a navigation index for the components layer. Cross-component interaction and runtime data flows are in [`architecture`](../architecture/INDEX.md), not here.
